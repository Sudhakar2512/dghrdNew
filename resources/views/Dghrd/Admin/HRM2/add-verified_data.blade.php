...
<x-layout>
    ...
    <div class="content-body">
        ...
        <div class="container-fluid">
            ...
            <h3 class="page-title">Verified Data
            </h3>
            <!-- row -->
...
@if ($value)
    <form method="post" action="{{ url('add-verified_data') . '/' . $value->id ?? '' }}" enctype="multipart/form-data">
@else
    <form method="post" action="{{ url('add-verified_data') }}" enctype="multipart/form-data">
@endif
@csrf
<div class="mb-3">
    <div class="form-group">
      ...
      <select class="form-select" name="tab" required>
        ...
        <option value="1" @if($value && $value->tab == "1") selected @endif>List of officers whose verified data awaited</option>
        <option value="2" @if($value && $value->tab == "2") selected @endif>Performa</option>
            ...
            <div class="col-12">
                <div class="thm-card">
                    <div class="thm-card-head">
                        <h4 class="thm-card-title"> <i class="thm-card-title-icon ti-file"></i> Verified Data
                            Table</h4>
                    </div>
                    <!-- /.box-header -->
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data as $key => $data)
                                        <tr>
                                            ...
                                            <td>
                                                @if($data->tab =='1' ?? '')
                                                List of officers whose verified data awaited
                                                @elseif($data->tab =='2' ?? '')
                                                Performa
                                                                                            @endif
                                            </td>
                                            ...
                                            <td>{{ $data->date ?? '' }}</td>
                                            <td>
                                                <a href="{{ url('verified_data-delete') . '/' . $data->id ?? '' }}"><button
                                                        type="button" class="btn btn-danger">
                                                        <i class="ti-trash" style="font-size: 18px;"></i>
                                                    </button></a>
                                                <a href="{{ url('add-verified_data') . '/' . $data->id ?? '' }}"><button
