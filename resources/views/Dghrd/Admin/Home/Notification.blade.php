...
@if ($value)
    <form method="post" action="{{ url('notification') . '/' . $value->id ?? '' }}" enctype="multipart/form-data">
@else
    <form method="post" action="{{ url('notification') }}" enctype="multipart/form-data">
@endif
@csrf
<div class="mb-3 ">
    ...
    <input type="text" id="simpleinput"
        class="form-control
    input-group" name="heading"  value="{{ $value->heading ?? '' }}"required>
    @error('heading')
            ...
            <div class="col-12">
                <div class="thm-card">
                    <div class="thm-card-head">
                        <h4 class="thm-card-title"><i class="thm-card-title-icon ti-bell"></i>Notification Table</h4>
                    </div>
                    <!-- /.box-header -->
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data as $key => $data)
                                        <tr>
                                            ...
                                            <td>{{ ucwords($data->heading) ?? '' }}</td>

                                            <td>
                                                <a href="{{ url('notification-delete') . '/' . $data->id ?? '' }}"><button
                                                        type="button" class="btn btn-danger">
                                                        <i class="ti-trash" style="font-size: 18px;"></i>
                                                    </button></a>
                                                <a href="{{ url('notification') . '/' . $data->id ?? '' }}"><button
