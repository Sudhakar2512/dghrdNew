...
<div class="col-md-8"> <!-- Increased width to col-md-8 -->
    @if ($value)
    <form method="post"
        action="{{ url('add-group-c') . '/' . $value->id ?? '' }}"
        enctype="multipart/form-data">
    @else
        <form method="post" action="{{ url('add-group-c') }}"
            enctype="multipart/form-data">
@endif
@csrf
<div class="mb-3">
    ...
    <div class="form-group">
        ...
        <select class="form-select" name="sub_tab" required>
            ...
            <option value="1" @if($value && $value->sub_tab == "1") selected @endif>
                RECRUITMENT RULES OF ASSISTANT SUB INSPECTOR WEAPONS</option>
            <option value="2"
                @if ($value && $value->sub_tab  == "2") selected @endif>
                RECRUITMENT RULES OF DOS</option>
            <option value="3" @if ($value && $value->sub_tab  == '3') selected @endif>
                RECRUITMENT RULES OF EXAMINER</option>
            <option value="4"
                @if($value && $value->sub_tab == "4") selected @endif>
               RECRUITMENT RULES OF DRIVERS</option>
            <option value="5"
                @if($value && $value->sub_tab == '5') selected @endif>
                RECRUITMENT RULES OF HAVALDAR & HEAD HAVALDAR</option>
            <option value="6"
                @if ($value && $value->sub_tab == '6') selected @endif>
                RECRUITMENT RULES OF LDC</option>
            <option value="7"
                @if ($value && $value->sub_tab == '7') selected @endif>
                RECRUITMENT RULES OF MTS</option>
            <option value="8"
                @if ($value && $value->sub_tab == '8') selected @endif>
                RECRUITMENT RULES OF RECORD KEEPER</option>
            <option value="9"
                @if ($value && $value->sub_tab == '9') selected @endif>
                RECRUITMENT RULES OF SENIOR TAX ASSISTANT</option>
            <option value="10"
                @if ($value && $value->sub_tab == '10') selected @endif>
                RECRUITMENT RULES OF STENOGRAPHER GR-II</option>
            <option value="11"
                @if ($value && $value->sub_tab == '11') selected @endif>
               RECRUITMENT RULES OF TAX ASSISTANT</option>
            ...
            <div class="col-12">
                <div class="thm-card">
                    <div class="thm-card-head">
                        <h4 class="thm-card-title"> <i class="thm-card-title-icon ti-file"></i>Group C Table</h4>
                    </div>
                    <!-- /.box-header -->
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data as $key => $data)
                                        <tr>
                                            ...
                                            <td>{{ $data->date ?? '' }}</td>
                                            <td>
                                                <a href="{{ url('group-c-delete') . '/' . $data->id ?? '' }}"><button
                                                        type="button" class="btn btn-danger">
                                                        <i class="ti-trash" style="font-size: 18px;"></i>
                                                    </button></a>
                                                <a href="{{ url('add-group-c') . '/' . $data->id ?? '' }}"><button
