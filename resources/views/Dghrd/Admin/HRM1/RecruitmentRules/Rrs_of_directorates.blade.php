...
<x-layout>
    ...
    <div class="content-body">
        ...
        <div class="container-fluid">
            ...
            <h3 class="page-title">RRs of DIRECTORATES

            </h3>
            <!-- row -->
            <div class="card mb-4 thm-card">
                <div class="card-header thm-card-head">
                    <h4 class="card-title thm-card-title"> <i class="thm-card-title-icon ti-file"></i>Form step</h4>
                </div>
...
<div class="col-md-8"> <!-- Increased width to col-md-8 -->
    @if ($value)
    <form method="post" action="{{ url('add-rrs-of-directorates') . '/' . $value->id ?? '' }}"
        enctype="multipart/form-data">
        @else
<form method="post" action="{{url('add-rrs-of-directorates')}}" enctype="multipart/form-data">
    @endif
    @csrf
    <div class="mb-3">
        <div class="form-group">
          <label class="form-label">Heading</label>
          <select class="form-select" name="sub_tab" required>
            <option value="">Choose Heading...</option>
            <option value="1" @if($value && $value->sub_tab == '1' ) selected @endif>GROUP-C POSTS</option>
            <option value="2" @if($value && $value->sub_tab == '2') selected @endif>RECRUITMENT RULES OF ADDITIONAL ASSISTANT DIRECTOR</option>
            <option value="3" @if($value && $value->sub_tab == '3') selected @endif>RECRUITMENT RULES OF STENOGRAHER GR-I</option>
            <option value="4" @if($value && $value->sub_tab == '4') selected @endif>TAX ASSISTANT</option>
            <option value="5" @if($value && $value->sub_tab  == '5') selected @endif>EXECUTIVE ASSISTANT</option>
            <option value="6" @if($value && $value->sub_tab  == '6') selected @endif>STAFF CAR DRIVER(SPECIAL GRADE)</option>
          </select>
        </div>
        @error('sub_tab')
...
<div class="mt-3">
    <button type="submit" class="btn thm-btn">
...
{{-- Table --}}
            ...
            <div class="col-12">
                <div class="thm-card">
                    <div class="thm-card-head">
                        <h4 class="thm-card-title"> <i class="thm-card-title-icon ti-file"></i>RRs of DIRECTORATES Table</h4>
                    </div>
                    <!-- /.box-header -->
                                    ...
                                    @foreach ($data as $key => $data)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                RECRUITMENT RULES OF ADDITIONAL ASSISTANT DIRECTOR
                                                @elseif($data->sub_tab =='3' ?? '')
                                                RECRUITMENT RULES OF STENOGRAHER GR-I
 ...
 <td>{{ ucwords($data->tittle) ?? '' }}</td>
                                            <td>{{ $data->date ?? '' }}</td>
                                            <td>
                                                <a href="{{ url('rrs-of-directorates-delete') . '/' . $data->id ?? '' }}"><button
                                                        type="button" class="btn btn-danger">
                                                        <i class="ti-trash" style="font-size: 18px;"></i>
                                                    </button></a>
                                                <a href="{{ url('add-rrs-of-directorates') . '/' . $data->id ?? '' }}"><button
