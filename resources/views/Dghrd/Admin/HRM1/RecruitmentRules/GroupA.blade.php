...
<div class="col-md-8"> <!-- Increased width to col-md-8 -->
    @if ($value)
    <form method="post"
        action="{{ url('add-group-a') . '/' . $value->id ?? '' }}"
        enctype="multipart/form-data">
    @else
        <form method="post" action="{{ url('add-group-a') }}"
            enctype="multipart/form-data">
@endif
@csrf
<div class="mb-3">
    ...
    <div class="form-group">
        ...
        <select class="form-select" name="subtab"  required>
            ...
            <option value="1" @if($value && $value->sub_tab  == '1' ) selected @endif>RECRUITMENT RULES OF ASSISTANT DIRECTOR (OL)</option>
            <option value="2" @if($value && $value->sub_tab == '2') selected @endif>RECRUITMENT RULES OF CAO</option>
            <option value="3" @if($value && $value->sub_tab == '3') selected @endif>RECRUITMENT RULES OF DEPUTY DIRECTOR (OL)</option>
            <option value="4" @if($value && $value->sub_tab == '4') selected @endif>RECRUITMENT RULES OF IRS</option>
          </select>
    </div>
    @error('subtab')
            ...
            <div class="col-12">
                <div class="thm-card">
                    <div class="thm-card-head">
                        <h4 class="thm-card-title"> <i class="thm-card-title-icon ti-file"></i>Group A Table</h4>
                    </div>
                    <!-- /.box-header -->
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data as $key => $data)
                                        <tr>
                                            ...
                                            <td>
                                                @if($data->sub_tab =='1' ?? '')
                                                RECRUITMENT RULES OF ASSISTANT DIRECTOR (OL)
                                                @elseif($data->sub_tab =='2' ?? '')
                                                RECRUITMENT RULES OF CAO
                                                @elseif($data->sub_tab =='3' ?? '')
                                                RECRUITMENT RULES OF DEPUTY DIRECTOR (OL)
                                                @elseif($data->sub_tab =='4' ?? '')
                                                RECRUITMENT RULES OF IRS
                                                @endif
                                            </td>
                                            <td>{{ ucwords($data->tittle) ?? '' }}</td>
                                            <td>{{ $data->date ?? '' }}</td>
                                            <td>
                                                <a href="{{ url('group-a-delete') . '/' . $data->id ?? '' }}"><button
                                                        type="button" class="btn btn-danger">
                                                        <i class="ti-trash" style="font-size: 18px;"></i>
                                                    </button></a>
                                                <a href="{{ url('add-group-a') . '/' . $data->id ?? '' }}"><button
