...
<div class="col-md-8"> <!-- Increased width to col-md-8 -->
    @if ($value)
                                                <form method="post" action="{{ url('add-dppr') . '/' . $value->id ?? '' }}"
                                                    enctype="multipart/form-data">
                                                    @else
                                            <form method="post" action="{{url('add-dppr')}}" enctype="multipart/form-data">
                                                @endif
                                                @csrf
                                                <div class="mb-3">
                                                    <div class="form-group">
                                                      ...
                                                      <select class="form-select" name="sub_tab" required>
                                                        ...
                                                        <option value="1" @if($value && $value->sub_tab == '1' ) selected @endif>GR C and D posts</option>
                                                        <option value="2" @if($value && $value->sub_tab == '2') selected @endif>MANAGER PRESS</option>
...
{{-- Table --}}
            ...
            <div class="col-12">
                <div class="thm-card">
                    <div class="thm-card-head">
                        <h4 class="thm-card-title"> <i class="thm-card-title-icon ti-file"></i> DPPR Table</h4>
                    </div>
                    <!-- /.box-header -->
 ...
 <td>{{ ucwords($data->tittle) ?? '' }}</td>
                                            <td>{{ $data->date ?? '' }}</td>
                                            <td>
                                                <a href="{{ url('dppr-delete') . '/' . $data->id ?? '' }}"><button
                                                        type="button" class="btn btn-danger">
                                                        <i class="ti-trash" style="font-size: 18px;"></i>
                                                    </button></a>
                                                <a href="{{ url('add-dppr') . '/' . $data->id ?? '' }}"><button
