...
<x-layout>
    ...
    <div class="content-body">
        ...
        <div class="container-fluid">
            ...
            <h3 class="page-title">CENTRAL BUREAU OF NARCOTICS(CBN)
...
<div class="col-md-8"> <!-- Increased width to col-md-8 -->
    @if ($value)
    <form method="post" action="{{ url('add-cbn') . '/' . $value->id ?? '' }}"
        enctype="multipart/form-data">
        @else
<form method="post" action="{{url('add-cbn')}}" enctype="multipart/form-data">
    @endif
    @csrf
    <div class="mb-3">
        <div class="form-group">
          ...
          <select class="form-select" name="sub_tab" required>
            ...
            <option value="1" @if($value && $value->sub_tab == '1' ) selected @endif>DRIVER SPECIAL GRADE</option>
            <option value="2" @if($value && $value->sub_tab == '2') selected @endif>GR-C DRIVER</option>
            <option value="3" @if($value && $value->sub_tab == '3') selected @endif>GROUP-B POSTS</option>
            <option value="4" @if($value && $value->sub_tab == '4') selected @endif>GROUP-C CBN</option>
            <option value="5" @if($value && $value->sub_tab  == '5') selected @endif>MTS CBN</option>
          </select>
        </div>
        @error('sub_tab')
...
{{-- Table --}}
            ...
            <div class="col-12">
                <div class="thm-card">
                    <div class="thm-card-head">
                        <h4 class="thm-card-title"><i class="thm-card-title-icon ti-file"></i> CENTRAL BUREAU OF NARCOTICS(CBN) Table</h4>
                    </div>
                    <!-- /.box-header -->
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data as $key => $data)
                                        <tr>
                                            ...
                                            <td>{{ $data->date ?? '' }}</td>
                                            <td>
                                                <a href="{{ url('cbn-delete') . '/' . $data->id ?? '' }}"><button
                                                        type="button" class="btn btn-danger">
                                                        <i class="ti-trash" style="font-size: 18px;"></i>
                                                    </button></a>
                                                <a href="{{ url('add-cbn') . '/' . $data->id ?? '' }}"><button
