...
@if ($value)
    <form method="post" action="{{ url('add-sparrow') . '/' . $value->id ?? '' }}" enctype="multipart/form-data">
@else
    <form method="post" action="{{ url('add-sparrow') }}" enctype="multipart/form-data">
@endif
@csrf
<div class="mb-3">
    <div class="form-group">
      ...
      <select class="form-select" name="tab" required>
        ...
        <option value="1" @if($value && $value->tab == "1") selected @endif>Online writing of APARs</option>
        <option value="2" @if($value && $value->tab == "2") selected @endif>Online writing of IPRs</option>
        <option value="3" @if($value && $value->tab == '3') selected @endif>SPARROW help manual</option>
            ...
            <div class="col-12">
                <div class="thm-card">
                    <div class="thm-card-head">
                        <h4 class="thm-card-title"> <i class="thm-card-title-icon ti-file"></i>SPARROW Table</h4>
                    </div>
                    <!-- /.box-header -->
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data as $key => $data)
                                        <tr>
                                            ...
                                            <td>
                                                @if($data->tab =='1' ?? '')
                                                Online writing of APARs
                                                @elseif($data->tab =='2' ?? '')
                                                Online writing of IPRs
                                                @elseif($data->tab =='3' ?? '')
                                                SPARROW help manual
                                                @endif
                                            </td>
                                            ...
                                            <td>
                                                @php
                                                    $title = ucwords($data->tittle) ?? '';
                                                    echo wordwrap($data->tittle, 50, "<br>\n", true);
                                                @endphp
                                            </td>
                                                                                        <td>{{ $data->date ?? '' }}</td>
                                            <td>
                                                <a href="{{ url('sparrow-delete') . '/' . $data->id ?? '' }}"><button
                                                        type="button" class="btn btn-danger">
                                                        <i class="ti-trash" style="font-size: 18px;"></i>
                                                    </button></a>
                                                <a href="{{ url('add-sparrow') . '/' . $data->id ?? '' }}"><button
