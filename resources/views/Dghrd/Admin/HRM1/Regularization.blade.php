<x-layout>
    <div class="content-body">
        ...
        <h3 class="page-title">Regularization</h3>
        <!-- row -->
            ...
            <div class="card-body thm-card-body">
                <div class="row">
                    <div class="col-md-8"> <!-- Increased width to col-md-8 -->
                        @if ($value)
                            <form method="post" action="{{ url('update-regularization') . '/' . $value->id ?? '' }}" enctype="multipart/form-data">
                        @else
                            <form method="post" action="{{ url('submit-regularization') }}" enctype="multipart/form-data">
                        @endif
                        @csrf
                        <div class="mb-3">
                            <label class="text-label form-label">Title</label>
                            <input type="text" class="form-control" name="title" placeholder="Write Title for PDF" value="{{ $value->title ?? '' }}">
                            @error('title')
        ...
        <div class="col-12">
            <div class="thm-card">
                <div class="thm-card-head">
                    <h4 class="thm-card-title"> <i class="thm-card-title-icon ti-file"></i> Regularization Table</h4>
                </div>
                <!-- /.box-header -->
                                ...
                                @foreach ($data as $key => $data)
                                    <tr>
                                        ...
                                        <td class="text-dark">
                                            ...
                                            @else
                                                <i class="fa fa-exclamation-triangle" aria-hidden="true"></i><span>No Pdf</span>
                                            @endif
                                        </td>
                                        <td>
                                            @php
                                                $title = ucwords($data->title) ?? '';
                                                echo wordwrap($data->title, 50, "<br>\n", true);
                                            @endphp
                                        </td>
                                        <td>{{ $data->date ?? '' }}</td>
                                        <td>
                                            <a href="{{ url('regularization-delete') . '/' . $data->id ?? '' }}"><button type="button" class="btn btn-danger"><i class="ti-trash" style="font-size: 18px;"></i></button></a>
                                            <a href="{{ url('add-regularization') . '/' . $data->id ?? '' }}"><button type="button" class="btn btn-primary " > <i class="ti-pencil" style="font-size: 18px;"></i></button></a>
                                        </td>
                                    </tr>
